...
void clear_screen(void) {
  ...
  for(p = 0; p < 8; p++) {
    ...
    for(c = 0; c < 129; c++) {
      lcd_command(CMD_SET_COLUMN_LOWER | (c & 0xf));		// Last 4 bit contains the least significant 4 bits of col address
      lcd_command(CMD_SET_COLUMN_UPPER | ((c >> 4) & 0xf));	// // Last 4 bit contains the most significant 4 bits of col address
      lcd_data(0x0);
    }     
  }
}
...
// Only horizontal and vertical lines possible
...
void clearline(uint8_t *buff,uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1,uint8_t color) {
	...
	if (x0 == x1)	// Vertical line
	{
		length = y1 - y0;
		for(int i = 0; i <= length; i++)
			clearpixel(buff, x0, y0 + i);
	}
	else if(y0 == y1)	// Horizontal line
	{
		length = x1 - x0;
		for(int i = 0; i <= length; i++)
			clearpixel(buff, x0 + i, y0);
	}
}
...
void fillrect(uint8_t *buff,uint8_t x, uint8_t y, uint8_t w, uint8_t h,uint8_t color) {
	for(uint8_t i = 0; i < h; i++)
		drawline(buff, x, y + i, x + w, y + i, color); 	
}
...
void fillcircle(uint8_t *buff,uint8_t x0, uint8_t y0, uint8_t radius,uint8_t color) {
		int x = radius;
		int y = 0;
		...
		while (x >= y)
		{
			...
			{
				setpixel(buff, i, y0 + y);
				setpixel(buff, i, y0 - y);
			}
			for (int i = x0 - y; i <= x0 + y; i++)
			{
				setpixel(buff, i, y0 + x);
				setpixel(buff, i, y0 - x);
			}
